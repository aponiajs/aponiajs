...
test("accepts a body matching the Standard Schema validator", async () => {
  const response = await send(application, "/items", "POST", {
    name: "Anvil",
    quantity: 2,
  });
...
test("rejects a body that violates a field constraint", async () => {
  const response = await send(application, "/items", "POST", { name: "A", quantity: 2 });
...
test("rejects a body missing a required field", async () => {
  const response = await send(application, "/items", "POST", { name: "Anvil" });

  expect(response.status).toBe(422);
});

test("accepts a query matching the platform-native validator", async () => {
  await send(application, "/items", "POST", { name: "Rope", quantity: 5 });
  const response = await get(application, "/items?term=rop&take=1");
...
test("rejects a query missing a required parameter", async () => {
  expect((await get(application, "/items")).status).toBe(422);
});

test("rejects a query parameter that fails its constraint", async () => {
  expect((await get(application, "/items?term=")).status).toBe(422);
});

test("validates headers through the same slot mechanism", async () => {
  const accepted = await get(application, "/items/tenant", {
    headers: { "x-tenant": "acme" },
  });
  const rejected = await get(application, "/items/tenant");
...
test("never runs the handler for a rejected request", async () => {
  const before = await get(application, "/items?term=never");
  await send(application, "/items", "POST", { name: "n", quantity: 1 });
  const after = await get(application, "/items?term=never");
