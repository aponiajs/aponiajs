...
test("serves a controller built from hand-written descriptors", async () => {
  ...
  expect(await response.json()).toMatchObject({ namespace: "descriptors" });
});
...
test("builds an application whose root module is itself a descriptor", async () => {
  expect((await get(application, "/metrics")).status).toBe(200);
});
