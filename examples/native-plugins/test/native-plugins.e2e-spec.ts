...
test("reaches every plugin route through the mounted controller", async () => {
  expect((await get(application, "/plugins")).status).toBe(200);
  expect((await get(application, "/plugins/plugin-local")).status).toBe(200);
});
